...
async function initDatabase() {
  ...
  for (const staff of shiftStaffList) {
    const existing = await pool.query(
      "SELECT * FROM shift_staff WHERE name = $1",
      [staff.name],
    );
    if (existing.rows.length === 0) {
      await pool.query(
...
app.post("/api/login", async (req, res) => {
  ...
  if (user.role === "operator") {
    ...
    const staffResult = await pool.query(
      "SELECT * FROM shift_staff WHERE id = $1",
      [shiftStaffId],
    );
...
app.get("/api/journal", authenticateToken, async (req, res) => {
  const result = await pool.query(
    "SELECT * FROM journal_entries ORDER BY date DESC, start_time DESC",
  );
  res.json(result.rows);
});
...
initDatabase()
  .then(() => {
    app.listen(PORT, HOST, () => {
      ...
      console.log(
        `👥 Пользователи: chief_engineer/eng123, power_engineer/power123, lead_engineer/lead123, master/master123, operator/operator123`,
      );

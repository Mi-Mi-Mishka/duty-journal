...
(function() {
    ...
    function updateStaffDisplay() {
        ...
        if (emergencyStaffInput) emergencyStaffInput.value = window.currentStaffName;
    }
    
    function updateButtonsVisibility() {
        const addEntryButton = document.querySelector('#eventTypeDropdown')?.closest('.dropdown');
    ...
    function formatInspectionReadings(readings) {
        ...
        if (lvsrReadings.length) result.push(`🖥️ Температуры: ${lvsrReadings.join(', ')}`);
    ...
    function getEventTypeBadge(type) {
        switch(type) {
            case EVENT_TYPES.INSPECTION: return '<span class="event-type-badge event-type-inspection">Обход</span>';
            case EVENT_TYPES.EMERGENCY: return '<span class="event-type-badge event-type-emergency">Авария</span>';
            case EVENT_TYPES.SHIFT: return '<span class="event-type-badge event-type-shift">Смена</span>';
            default: return '<span class="event-type-badge event-type-other">Прочее</span>';
        }
    }
    ...
    window.insertEmergencyTemplate = function(templateNumber) {
        ...
        switch(templateNumber) {
            case 1: typeSelect.value = 'power'; description.value = 'Произошло отключение электроэнергии.'; break;
            case 2: typeSelect.value = 'water'; description.value = 'Прорыв трубы, подтопление помещения.'; break;
            case 3: typeSelect.value = 'fire'; description.value = 'Возникло задымление. Проведена эвакуация.'; break;
            case 4: typeSelect.value = 'equipment'; description.value = 'Отказ оборудования.'; break;
            default: return;
        }
        window.showNotification('Шаблон вставлен', 'info');
    };
    ...
    function getEmergencyTypeName(type) {
        const types = { equipment: 'Отказ оборудования', power: 'Отключение электроэнергии', water: 'Авария водоснабжения', fire: 'Пожар / Задымление', gas: 'Утечка газа' };
    ...
    function updateShiftModalStaff() {
        ...
        if (shiftStaffDisplay) shiftStaffDisplay.textContent = window.currentStaffName || 'Не выбран';
    }
    ...
    function attachEventHandlers() {
        ...
        document.getElementById('emergencyForm')?.addEventListener('submit', addEmergencyEntry);
        document.getElementById('confirmShiftBtn')?.addEventListener('click', createShiftEntry);
        document.getElementById('addInspectionEvent')?.addEventListener('click', (e) => { e.preventDefault(); showInspectionModal(); });
        document.getElementById('addEmergencyEvent')?.addEventListener('click', (e) => { e.preventDefault(); showEmergencyModal(); });
        ...
        document.getElementById('shiftModal')?.addEventListener('show.bs.modal', async function() {
            ...
            const hours = String(now.getHours()).padStart(2, '0');
            const minutes = String(now.getMinutes()).padStart(2, '0');
            document.getElementById('shiftDatetime').value = `${year}-${month}-${day}T${hours}:${minutes}`;
        ...
        document.getElementById('inspectionModal')?.addEventListener('show.bs.modal', function(event) {
            ...
            document.getElementById('inspectionStaffName').value = window.currentStaffName;
        });
        
        document.getElementById('emergencyModal')?.addEventListener('show.bs.modal', function(event) {
            if (!window.currentStaffName) {
                event.preventDefault();
                alert('Выберите текущего дежурного');
                return false;

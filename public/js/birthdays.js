...
(function() {
    ...
    async function renderBirthdaysList(filterMonth = 'all') {
        ...
        if (birthdays.length === 0) {
            list.innerHTML = '<div class="col-12"><div class="text-center py-5"><i class="bi bi-gift fs-1 text-muted"></i><p class="text-muted mt-3">Нет добавленных сотрудников.</p></div></div>';
            return;
        }
        ...
        let filtered = sorted;
        if (filterMonth !== 'all') {
            filtered = sorted.filter(b => new Date(b.birth_date).getMonth() + 1 === parseInt(filterMonth));
        }
        
        if (filtered.length === 0) {
            list.innerHTML = '<div class="col-12"><div class="text-center py-5"><i class="bi bi-calendar-x fs-1 text-muted"></i><p class="text-muted mt-3">Нет дней рождения в выбранном месяце.</p></div></div>';
            return;
        }
        
        const monthNames = ['Января', 'Февраля', 'Марта', 'Апреля', 'Мая', 'Июня', 'Июля', 'Августа', 'Сентября', 'Октября', 'Ноября', 'Декабря'];
        let currentMonth = -1, html = '';
